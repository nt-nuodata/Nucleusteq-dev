...
df_14=spark.sql("""SELECT TO_CHAR(START_TSTMP_output, 'MM/DD/YYYY HH24:MI:SS') AS START_TSTMP,
TO_CHAR(SYSDATE, 'MM/DD/YYYY HH24:MI:SS') AS END_TSTMP,
...
IIF(DUPLICATE_ROW_CNT>0, 'There are duplicate records in the table', SQLError) AS SQL_TRANSFORM_ERROR,
Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM SQL_INS_and_DUPS_CHECK_13""")
...
df_16=spark.sql("""SELECT START_TSTMP AS START_TSTMP,
...
Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM AGG_15""")
...
df_18=spark.sql("""SELECT DUPLICATE_ROW_CNT_output AS DUPLICATE_ROW_CNT,
SQL_TRANSFORM_ERROR_output AS SQL_TRANSFORM_ERROR,
IIF(TO_INTEGER(DUPLICATE_ROW_CNT_output)> 0, ABORT('There are duplicates rows in the table'), IIF( NOT ISNULL (SQL_TRANSFORM_ERROR_output), ABORT('There is an error in the INSERT statement'))) AS ABORT_SESSION,
Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM SQL_INS_to_SQL_TRANSFORM_LOG_17""")
