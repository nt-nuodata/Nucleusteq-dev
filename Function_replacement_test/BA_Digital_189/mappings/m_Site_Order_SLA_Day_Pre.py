...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Order_SLA_Day_Pre")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_Site_Order_SLA_Day_Pre", variablesTableName, mainWorkflowId)
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_HOURS_DAY2_2
...
df_2.createOrReplaceTempView("Shortcut_to_SITE_HOURS_DAY2_2")

# COMMAND ----------
# DBTITLE 1, SQ_Shortcut_to_SITE_HOURS_DAY1_3
...
query_3 = f"""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SITE_HOURS_DAY2_2"""

df_3 = spark.sql(query_3)

df_3.createOrReplaceTempView("SQ_Shortcut_to_SITE_HOURS_DAY1_3")

# COMMAND ----------
# DBTITLE 1, Jnr_Site_Hours_Day_For_Next_Day_4
...
FROM
  SQ_Shortcut_to_SITE_HOURS_DAY1_3 MASTER
...
df_4.createOrReplaceTempView("Jnr_Site_Hours_Day_For_Next_Day_4")
...
query_5 = f"""SELECT
  ...
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  Jnr_Site_Hours_Day_For_Next_Day_4
ORDER BY
  DAY_DT ASC,
  LOCATION_ID ASC,
  BUSINESS_AREA ASC,
...
df_6.createOrReplaceTempView("Fil_Close_Flag_6")

# COMMAND ----------
# DBTITLE 1, Agg_Next_Day_7
...
df_7.createOrReplaceTempView("Agg_Next_Day_7")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_HOURS_DAY_8
...
df_8.createOrReplaceTempView("Shortcut_to_SITE_HOURS_DAY_8")

# COMMAND ----------
# DBTITLE 1, SQ_Shortcut_to_SITE_HOURS_DAY3_9
...
query_9 = f"""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SITE_HOURS_DAY_8"""

df_9 = spark.sql(query_9)

df_9.createOrReplaceTempView("SQ_Shortcut_to_SITE_HOURS_DAY3_9")
...
query_12 = f"""SELECT
  ...
  MASTER.Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  SQ_Shortcut_to_SITE_HOURS_DAY3_9 MASTER
...
WHERE
  ...
  AND (
    DAY_DT1 >= START_DT
    AND DAY_DT1 <= END_DT
  ) --(DAY_DT1 >= START_DT AND DAY_DT1 <= END_DT)
  --AND
  --(BUSINESS_AREA = 'Store'
  --  OR BUSINESS_AREA = 'DC'
  --  OR BUSINESS_AREA = 'Vendor')
  --AND (UPDATE_TSTMP1  >= TRUNC(now())
  --   OR UPDATE_TSTMP  >= TRUNC(now()))
  --((DAY_DT1 >= START_DT AND DAY_DT1 <= END_DT) or (UPDATE_TSTMP1 > ADD_TO_DATE(TRUNC(now()), 'DD', -1))) AND  (BUSINESS_AREA = 'Store' OR BUSINESS_AREA = 'DC'  OR BUSINESS_AREA = 'Vendor')
  --AND (UPDATE_TSTMP >= ADD_TO_DATE(TRUNC(now()), 'DD', -1) --OR UPDATE_TSTMP1 >= ADD_TO_DATE(TRUNC(now()),  'DD', -1) --)"""
...
# COMMAND ----------
# DBTITLE 1, SITE_ORDER_SLA_DAY_PRE
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Order_SLA_Day_Pre")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_Site_Order_SLA_Day_Pre", mainWorkflowId, parentName)
