...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_rank")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_sku_rank", variablesTableName, mainWorkflowId)
    ...
    FROM
      (
        SELECT
          ...
          SUM(
            S.SALES_QTY - S.RETURN_QTY - S.SPECIAL_SALES_QTY + S.SPECIAL_RETURN_QTY
          ) AS NQTY
        FROM
          SALES_DAY_SKU_STORE S,
          Shortcut_To_SKU_PROFILE_3 K,
          DM_DEPT_SEGMENTS D,
          SITE_PROFILE P
        WHERE
          S.WEEK_DT >= (
            (CURRENT_DATE + 1 - DATE_PART('DOW', CURRENT_DATE)) - 7 * 7
          )
...
WHERE
  ...
  AND TT.UNITS_STR_WK_VALUE BETWEEN NVL(RANK_MIN_VALUE, -1000000)
  AND NVL(RANK_MAX_VALUE, 1000000)"""
...
SELECT
  ...
  LOAD_DT AS LOAD_DT
FROM
  SQ_Shortcut_to_USER_RANKING_HIERARCHY_4""")

# COMMAND ----------
#Post session variable updation
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_rank")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_sku_rank", mainWorkflowId, parentName)
