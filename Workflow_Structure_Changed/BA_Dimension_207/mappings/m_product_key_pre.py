...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_product_key_pre")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_product_key_pre", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_To_PRODUCT_0
...
df_0.createOrReplaceTempView("Shortcut_To_PRODUCT_0")
...
query_1 = f"""SELECT
  ...
  SAP.BUYER_ID AS BUYER_ID,
  SAP.PRIMARY_UPC AS PRIMARY_UPC,
  SAP.SKU_NBR AS SKU_NBR,
  SAP.SAP_CATEGORY_ID AS SAP_CATEGORY_ID,
  SAP.OLD_ARTICLE_NBR AS OLD_ARTICLE_NBR,
  CASE
    WHEN SAP.OLD_ARTICLE_NBR IS NULL THEN SAP.OLD_ARTICLE_NBR
    ELSE substr(SAP.OLD_ARTICLE_NBR, 1, 6) || NVL(substr(DCLR_ID, 1, 3), '', substr(DCLR_ID, 1, 3)) || NVL(DSZE_ID, '', DSZE_ID) || NVL(DBRD_ID, '', DBRD_ID) || ' '
  END AS ITEM_CONCATENATED,
  SAP.PRIMARY_VENDOR_ID AS PRIMARY_VENDOR_ID,
  SAP.PURCH_GROUP_ID AS PURCH_GROUP_ID,
...
FROM
  (
    SELECT
      ...
      PRD.OLD_ARTICLE_NBR
    FROM
      Shortcut_To_PRODUCT_0 PRD
    WHERE
      PRD.DATE_PROD_REFRESHED = CURRENT_DATE
  ) SAP
  LEFT OUTER JOIN (
    SELECT
      ATT.PRODUCT_ID,
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_product_key_pre")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_product_key_pre", mainWorkflowId, parentName)
