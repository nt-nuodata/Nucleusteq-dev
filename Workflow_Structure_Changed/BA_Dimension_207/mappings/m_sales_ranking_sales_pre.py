...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sales_ranking_sales_pre")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_sales_ranking_sales_pre", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_to_USR_STORE_DIVISIONS_0
...
df_0.createOrReplaceTempView("Shortcut_to_USR_STORE_DIVISIONS_0")
...
FROM
  Shortcut_to_USR_STORE_DIVISIONS_0"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_RANKING_DATE_PRE_2
...
df_2.createOrReplaceTempView("Shortcut_to_SALES_RANKING_DATE_PRE_2")
...
query_3 = f"""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SALES_RANKING_DATE_PRE_2"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_DAY_SKU_STORE_RPT_4
...
df_4.createOrReplaceTempView("Shortcut_to_SALES_DAY_SKU_STORE_RPT_4")
...
query_5 = f"""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SALES_DAY_SKU_STORE_RPT_4"""
...
FROM
  ...
  INNER JOIN SQ_Shortcut_to_SALES_DAY_SKU_STORE_RPT_5 DETAIL ON MASTER.WEEK_DT = DETAIL.WEEK_DT"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SKU_PROFILE_7
...
df_7.createOrReplaceTempView("Shortcut_to_SKU_PROFILE_7")
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SLSCMP_STORE_9
...
df_9.createOrReplaceTempView("Shortcut_to_SLSCMP_STORE_9")
...
FROM
  ...
  INNER JOIN JNR_WEEKS_6 DETAIL ON MASTER.LOCATION_ID = DETAIL.LOCATION_ID"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_PROFILE_12
...
df_12.createOrReplaceTempView("Shortcut_to_SITE_PROFILE_12")
...
FROM
  ...
  INNER JOIN JNR_SLS_CMP_11 DETAIL ON MASTER.LOCATION_ID = DETAIL.LOCATION_ID"""
...
FROM
  ...
  INNER JOIN JNR_SITE_PROFILE_14 DETAIL ON MASTER.PRODUCT_ID = DETAIL.PRODUCT_ID"""
...
FROM
  ...
  INNER JOIN JNR_SKU_PROFILE_15 DETAIL ON MASTER.SAP_DEPT_ID = DETAIL.SAP_DEPT_ID"""
...
query_17 = f"""SELECT
  ...
  IFF (
    ...
    NET_SALES_AMT * EXCH_RATE_PCT,
    0
  ) AS CONSUMABLES_SALES,
  IFF (
    STORE_DIVISION_ID = 10,
    NET_SALES_AMT * EXCH_RATE_PCT,
    0
  ) AS HARDGOODS_SALES,
  IFF (
...
query_18 = f"""SELECT
  MAX(RANKING_WEEK_DT) AS RANKING_WEEK_DT1,
  LOCATION_ID AS LOCATION_ID,
  ROUND(SUM(TOTAL_SALES), 2) AS TOTAL_SALES,
  ROUND(SUM(MERCH_SALES), 2) AS MERCH_SALES,
  ROUND(SUM(SERVICES_SALES), 2) AS SERVICES_SALES,
  ROUND(SUM(SALON_SALES), 2) AS SALON_SALES,
  ROUND(SUM(TRAINING_SALES), 2) AS TRAINING_SALES,
  ROUND(SUM(HOTEL_DDC_SALES), 2) AS HOTEL_DDC_SALES,
  ROUND(SUM(CONSUMABLES_SALES), 2) AS CONSUMABLES_SALES,
  ROUND(SUM(HARDGOODS_SALES), 2) AS HARDGOODS_SALES,
  ROUND(SUM(SPECIALTY_SALES), 2) AS SPECIALTY_SALES,
  MAX(COMP_CURR_FLAG) AS COMP_CURR_FLAG,
  MAX(SALES_CURR_FLAG) AS SALES_CURR_FLAG,
  MAX(LOCATION_TYPE_ID) AS LOCATION_TYPE_ID,
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sales_ranking_sales_pre")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_sales_ranking_sales_pre", mainWorkflowId, parentName)
