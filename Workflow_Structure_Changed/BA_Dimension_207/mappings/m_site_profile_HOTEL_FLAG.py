...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_site_profile_HOTEL_FLAG")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_site_profile_HOTEL_FLAG", variablesTableName, mainWorkflowId)
...
query_1 = f"""SELECT
  AA.location_id AS LOCATION_ID,
  CASE
    WHEN (MAX(sp.store_open_close_flag) = 'C') THEN MAX(sp.HOTEL_FLAG)
    ELSE MAX(AA.hotel_flag)
  END AS HOTEL_FLAG,
...
FROM
  (
    ...
    WHERE
      (
        sdss.week_dt BETWEEN (
          CURRENT_DATE - (date_part('DOW', CURRENT_DATE) + ({p_opendays}))
        )
        AND (CURRENT_DATE - (date_part('DOW', CURRENT_DATE) - 1))
      )
      AND ({p_hotel})
    GROUP BY
      sdss.location_id,
  ...
  LEFT JOIN site_profile sp ON AA.location_id = sp.LOCATION_ID
GROUP BY
  AA.location_id
UNION
SELECT
...
USING
  UP_SITE_PROFILE_2 AS SOURCE ON TARGET.LOCATION_ID = SOURCE.LOCATION_ID
  WHEN MATCHED THEN
UPDATE
SET
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_site_profile_HOTEL_FLAG")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_site_profile_HOTEL_FLAG", mainWorkflowId, parentName)
