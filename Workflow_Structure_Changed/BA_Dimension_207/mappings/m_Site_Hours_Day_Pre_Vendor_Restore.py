...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Hours_Day_Pre_Vendor_Restore")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_Site_Hours_Day_Pre_Vendor_Restore", variablesTableName, mainWorkflowId)
...
WHERE
  ...
  AND DAY_DT < ADD_TO_DATE(TRUNC(now()), 'D', 15)"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_PROFILE_RPT_4
...
df_4.createOrReplaceTempView("Shortcut_to_SITE_PROFILE_RPT_4")
...
query_5 = f"""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SITE_PROFILE_RPT_4"""
...
FROM
  EXPTRANS_3 MASTER
  INNER JOIN EXPTRANS1_7 DETAIL ON MASTER.Join = DETAIL.Join"""
...
query_9 = f"""SELECT
  ...
  ADD_TO_DATE(ADD_TO_DATE(DAY_DT, 'D', 1), 'MS', -1) AS CLOSE_TSTMP,
  now() AS LOAD_TSTMP,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  JNRTRANS_8"""
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Hours_Day_Pre_Vendor_Restore")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_Site_Hours_Day_Pre_Vendor_Restore", mainWorkflowId, parentName)
