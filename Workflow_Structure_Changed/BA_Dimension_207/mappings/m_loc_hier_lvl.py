...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_loc_hier_lvl")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_loc_hier_lvl", variablesTableName, mainWorkflowId)
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_REGION_1
...
df_1.createOrReplaceTempView("Shortcut_to_REGION_1")
...
query_3 = f"""SELECT
  1 AS LOC_HIER_LVL,
  'Total' AS LOC_HIER_DESC,
  'Company' AS LOC_HIER_ELEMENT,
  DISTRICT_ID AS DISTRICT_ID,
...
SELECT
  2 AS LOC_HIER_LVL,
  'Country' AS LOC_HIER_DESC,
  TRIM(COUNTRY_CD) AS LOC_HIER_ELEMENT,
  DISTRICT_ID AS DISTRICT_ID,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
...
SELECT
  3 AS LOC_HIER_LVL,
  'Region' AS LOC_HIER_DESC,
  TRIM(R.REGION_DESC) AS LOC_HIER_ELEMENT,
  S.DISTRICT_ID AS DISTRICT_ID,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SITE_PROFILE_2 S,
  Shortcut_to_REGION_1 R
WHERE
...
SELECT
  4 AS LOC_HIER_LVL,
  'District' AS LOC_HIER_DESC,
  TRIM(D.DISTRICT_DESC) AS LOC_HIER_ELEMENT,
  S.DISTRICT_ID AS DISTRICT_ID,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_SITE_PROFILE_2 S,
  Shortcut_to_DISTRICT_0 D
WHERE
...
SELECT
  ...
  DISTRICT_ID AS DISTRICT_ID
FROM
  ASQ_District_Join_Region_Join_Site_Profile_3""")

# COMMAND ----------
#Post session variable updation
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_loc_hier_lvl")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_loc_hier_lvl", mainWorkflowId, parentName)
