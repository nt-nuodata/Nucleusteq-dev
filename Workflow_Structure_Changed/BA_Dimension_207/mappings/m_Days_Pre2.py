...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Days_Pre2")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_Days_Pre2", variablesTableName, mainWorkflowId)
...
from
  (
    SELECT
      DAYDT,
      CASE
        WHEN DATEPART (DW, DAYDT) IN (1, 7) -- SATURDAY AND SUNDAY
        THEN 'N'
        ELSE 'Y'
      ...
      CASE
        WHEN DATEPART (DW, DAYDT) - 1 = 0 -- SUNDAY
        THEN 7
      ...
      CAST (
        DATEPART (YY, DAYDT + 7 - (DATEPART (DW, DAYDT - 1))) AS VARCHAR (4)
      ) + REPLICATE (
        '0',
        2 - LEN(
          DENSE_RANK () OVER (
      ...
      CAST (DATEPART (YY, DAYDT) AS VARCHAR (4)) + REPLICATE ('0', 2 - LEN (DATEPART (MM, DAYDT))) + CAST (DATEPART (MM, DAYDT) AS VARCHAR (2)) AS CAL_MO,
      DATEPART (MM, DAYDT) AS CAL_MO_NBR,
      DATENAME (MM, DAYDT) AS CAL_MO_NAME,
      SUBSTRING (DATENAME (MM, DAYDT), 1, 3) AS CAL_MO_NAME_ABBR,
      CAST (DATEPART (YY, DAYDT) AS VARCHAR (4)) + REPLICATE ('0', 2 - LEN (DATEPART (QUARTER, DAYDT))) + CAST (DATEPART (QUARTER, DAYDT) AS VARCHAR (2)) AS CAL_QTR,
      DATEPART (QUARTER, DAYDT) AS CAL_QTR_NBR,
      CAST (DATEPART (YY, DAYDT) AS VARCHAR (4)) + (
        CASE
          WHEN DATEPART (QUARTER, DAYDT) <= 2 THEN '01'
          ELSE '02'
        END
      ) AS CAL_HALF,
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Days_Pre2")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_Days_Pre2", mainWorkflowId, parentName)
