...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_profile_subs_history_update")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_sku_profile_subs_history_update", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_To_SKU_SUBSTITUTION_0
...
df_0.createOrReplaceTempView("Shortcut_To_SKU_SUBSTITUTION_0")
...
query_2 = f"""SELECT
  PRODUCT_ID AS PRODUCT_ID,
  SUBS_HIST_FLAG AS SUBS_CURRENT,
  SUBS_CURR_FLAG AS SUBS_HISTORY,
...
FROM
  (
    SELECT
      ...
      MAX(SUBS_END_DT),
      CASE
        WHEN CURRENT_DATE BETWEEN MAX(SUBS_EFF_DT)
        AND MAX(SUBS_END_DT) THEN 'Y'
        ELSE 'N'
      ...
      CASE
        ...
        AND CURRENT_DATE BETWEEN MAX(SUBS_EFF_DT)
        AND MAX(SUBS_END_DT)
    ...
    FROM
      (
        SELECT
          ...
          SUBS_END_DT
        FROM
          Shortcut_To_SKU_SUBSTITUTION_0 SS
        UNION ALL
        SELECT
          SL.PRODUCT_ID,
          SL.LINK_PRODUCT_ID,
      ...
      Shortcut_To_SKU_PROFILE_1 SP1,
      Shortcut_To_SKU_PROFILE_1 SP2
    WHERE
...
SELECT
  ...
  LOAD_DT AS LOAD_DT
FROM
  ASQ_Shortcut_To_SKU_SUBSTITUTION_2""")
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_profile_subs_history_update")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_sku_profile_subs_history_update", mainWorkflowId, parentName)
