...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_profile_price_cost_update")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_sku_profile_price_cost_update", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_To_NATIONAL_PRICE_DAY_0
...
df_0.createOrReplaceTempView("Shortcut_To_NATIONAL_PRICE_DAY_0")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_CURRENCY_DAY_1
...
df_1.createOrReplaceTempView("Shortcut_to_CURRENCY_DAY_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_To_NATIONAL_COST_DAY_2
...
df_2.createOrReplaceTempView("Shortcut_To_NATIONAL_COST_DAY_2")
...
query_3 = f"""SELECT
  ...
  CASE
    ...
    ELSE MAX(NVL(S.NAT_PRICE_US_AMT, 0))
  END AS PRICE_US_AMT,
  CASE
    WHEN MAX(PRICE.NAT_PRICE_CA_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_CA_AMT)
    WHEN MAX(PRICE.NAT_PRICE_US_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_US_AMT)
    WHEN MAX(PRICE.NAT_PRICE_PR_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_PR_AMT)
    ELSE MAX(NVL(S.NAT_PRICE_CA_AMT, 0))
  END AS PRICE_CA_AMT,
  CASE
    WHEN MAX(PRICE.NAT_PRICE_PR_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_PR_AMT)
    WHEN MAX(PRICE.NAT_PRICE_US_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_US_AMT)
    WHEN MAX(PRICE.NAT_PRICE_CA_AMT) > 0 THEN MAX(PRICE.NAT_PRICE_CA_AMT)
    ELSE MAX(NVL(S.NAT_PRICE_PR_AMT, 0))
  END AS PRICE_PR_AMT,
  CASE
    WHEN MAX(COST.PRI_VEND_IND) = 1
    AND MAX(
      CASE
        WHEN COST.PRI_VEND_IND = 1 THEN NVL(COST.PURCH_COST_AMT, 0)
        ELSE 0
      END
    ) <> 0 THEN MAX(
      CASE
    ...
    WHEN MAX(NVL(COST.PURCH_COST_AMT, 0)) <> 0 THEN MAX(NVL(COST.PURCH_COST_AMT, 0))
    ELSE MAX(NVL(S.PURCH_COST_AMT, 0))
  END AS COST_AMT,
...
FROM
  ...
  LEFT OUTER JOIN (
    SELECT
      ...
      CASE
        WHEN SALES_ORG_CD = 'CA01' THEN ROUND(MAX(COND_AMT * C.EXCHANGE_RATE_PCNT), 2)
        ELSE 0
      END AS NAT_PRICE_CA_AMT
    FROM
      Shortcut_To_NATIONAL_PRICE_DAY_0,
      (
        SELECT
          EXCHANGE_RATE_PCNT
        FROM
    ...
    GROUP BY
      ...
      SALES_ORG_CD,
      COND_AMT
  ) PRICE ON S.SKU_NBR = PRICE.SKU_NBR
  LEFT OUTER JOIN (
    SELECT
      ...
      ROUND(
        CASE
          ...
          WHEN TRIM(NCD.COND_RT_UNIT) = 'CAD' THEN (
            NCD.COND_AMT / CAST(
              (
                CASE
      ...
      NCD.COND_TYPE_CD
    FROM
      Shortcut_To_NATIONAL_COST_DAY_2 NCD
      LEFT OUTER JOIN SKU_PROFILE SVD ON NCD.SKU_NBR = SVD.SKU_NBR
    ...
    WHERE
      ...
      AND (CURRENT_TIMESTAMP - INTERVAL '1 days') BETWEEN NCD.COND_EFF_DT
      AND NCD.COND_END_DT
      AND (
        TRIM(NCD.DELETE_IND) = ''
        OR TRIM(NCD.DELETE_IND) IS NULL
      )
  ) COST ON S.SKU_NBR = COST.SKU_NBR
GROUP BY
  S.PRODUCT_ID"""
...
# COMMAND ----------
# DBTITLE 1, Shortcut_To_SKU_PROFILE1_4
...
df_4.createOrReplaceTempView("Shortcut_To_SKU_PROFILE1_4")
...
SELECT
  ...
  PRIMARY_UPC_ID AS PRIMARY_UPC_ID,
  STATUS_ID AS STATUS_ID,
  SUBS_HISTORY AS SUBS_HIST_FLAG,
  SUBS_HIST_FLAG AS SUBS_HIST_FLAG,
  SUBS_CURRENT AS SUBS_CURR_FLAG,
  SUBS_CURR_FLAG AS SUBS_CURR_FLAG,
  SKU_DESC AS SKU_DESC,
  ALT_DESC AS ALT_DESC,
  SAP_CATEGORY_ID AS SAP_CATEGORY_ID,
  ...
  BUYER_ID AS BUYER_ID,
  PURCH_GROUP_ID AS PURCH_GROUP_ID,
  COST_AMT AS PURCH_COST_AMT,
  PRICE_US_AMT AS NAT_PRICE_US_AMT,
  TAX_CLASS_ID AS TAX_CLASS_ID,
  VALUATION_CLASS_CD AS VALUATION_CLASS_CD,
  BRAND_CD AS BRAND_CD,
  BRAND_CD AS BRAND_CD,
  ...
  OLD_ARTICLE_NBR AS OLD_ARTICLE_NBR,
  VENDOR_ARTICLE_NBR AS VENDOR_ARTICLE_NBR,
  DATE_INIT_MKDN AS INIT_MKDN_DT,
  DATE_DISC_START AS DISC_START_DT,
  DATE_ADDED AS ADD_DT,
  DATE_DELETED AS DELETE_DT,
  UPDATE_DT AS UPDATE_DT,
  ...
  FIRST_SALE_DT AS FIRST_SALE_DT,
  LAST_SALE_DT AS LAST_SALE_DT,
  OUT_DATE_FIRST_INV AS FIRST_INV_DT,
  OUT_DATE_LAST_INV AS LAST_INV_DT,
  LOAD_DT AS LOAD_DT,
  NULL AS BASE_NBR,
  NULL AS BP_COLOR_ID,
  NULL AS BP_SIZE_ID,
  ...
  CANADIAN_HTS_CD AS CANADIAN_HTS_CD,
  PRICE_CA_AMT AS NAT_PRICE_CA_AMT,
  PRICE_PR_AMT AS NAT_PRICE_PR_AMT,
  RTV_DEPT_CD AS RTV_DEPT_CD,
  GL_ACCT_NBR AS GL_ACCT_NBR,
  ARTICLE_CATEGORY_ID AS ARTICLE_CATEGORY_ID,
  COMPONENT_FLAG AS COMPONENT_FLAG,
  ...
  ZDISCO_STR_WRT_OFF_DT AS ZDISCO_STR_WRT_OFF_DT
FROM
  ASQ_Shortcut_To_NATIONAL_COST_DAY_3""")
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_sku_profile_price_cost_update")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_sku_profile_price_cost_update", mainWorkflowId, parentName)
