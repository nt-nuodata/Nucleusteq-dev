...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_USR_Store_Attributes_UPD_Dog_Walk_Flag")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_USR_Store_Attributes_UPD_Dog_Walk_Flag", variablesTableName, mainWorkflowId)
...
query_2 = f"""SELECT
  ...
  SStUSA1.DOG_WALK_FLAG AS DOG_WALK_FLAG,
...
WHERE
  IFF(
    ...
    AND SAP_CLASS_ID = 836 --Class that has all the Dog Walk related SKUs
,
    TRUE,
    FALSE
  )"""
...
query_11 = f"""SELECT
  LOCATION_ID AS LOCATION_ID,
  IFF(MAX_SALES >= 50, 1, 0) AS DOG_WALK_FLAG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  AGG_MAX_OF_WEEKS_10"""
...
query_13 = f"""SELECT
  ...
  IFF(
    STORE_OPEN_CLOSE_FLAG = 'C'
    OR IFF(ISNULL(DOG_WALK_FLAG1), 0, DOG_WALK_FLAG1) = IFF(ISNULL(DOG_WALK_FLAG), 0, DOG_WALK_FLAG),
    'DD_REJECT',
    'DD_UPDATE'
  ) AS UPD_STRATEGY,
  now() AS UPD_TSTMP,
...
query_14 = f"""SELECT
  LOCATION_ID AS LOCATION_ID,
  o_DOG_WALK_FLAG AS DOG_WALK_FLAG,
...
SET
  ...
  TARGET.DOG_WALK_FLAG = SOURCE.DOG_WALK_FLAG,
  TARGET.UPDATE_TSTMP = SOURCE.UPD_TSTMP""")

# COMMAND ----------
#Post session variable updation
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_USR_Store_Attributes_UPD_Dog_Walk_Flag")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_USR_Store_Attributes_UPD_Dog_Walk_Flag", mainWorkflowId, parentName)
