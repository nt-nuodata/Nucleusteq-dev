...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Dm_Hist")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_Site_Dm_Hist", variablesTableName, mainWorkflowId)
...
query_3 = f"""SELECT
  SDH.LOCATION_ID AS LOCATION_ID,
  SDH.SITE_DM_EFF_DT AS SITE_DM_EFF_DT,
  SDH.DIST_MGR_NAME AS DIST_MGR_NAME,
  SDH.SITE_DM_END_DT AS SITE_DM_END_DT,
  SDH.CURRENT_DIST_MGR AS CURRENT_DIST_MGR,
  SDH.UPDATE_TSTMP AS UPDATE_TSTMP,
  SDH.LOAD_TSTMP AS LOAD_TSTMP,
  EDF2.LOCATION_ID AS NEW_LOCATION_ID1,
  EDF2.DIST_MGR_NAME AS NEW_DIST_MGR_NAME1,
  EDF2.Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  EXP_DATES_FORMAT_2 EDF2
  LEFT JOIN SITE_DM_HIST SDH ON SDH.LOCATION_ID = EDF2.LOCATION_ID"""
...
query_4 = f"""SELECT
  ...
  TRUNC(ADD_TO_DATE(now(), 'DD', -1)) AS SITE_DM_EFF_DT,
  NEW_DIST_MGR_NAME1 AS NEW_DIST_MGR_NAME1,
  TO_DATE('12/31/9999', 'MM/DD/YYYY') AS SITE_DM_END_DT,
...
USING
  UPDTRANS_INSERT_REJECT_6 AS SOURCE ON TARGET.LOCATION_ID = SOURCE.NEW_LOCATION_ID1
  AND TARGET.SITE_DM_EFF_DT = SOURCE.SITE_DM_EFF_DT
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_UPDATE" THEN
UPDATE
SET
  TARGET.LOCATION_ID = SOURCE.NEW_LOCATION_ID1,
  TARGET.SITE_DM_EFF_DT = SOURCE.SITE_DM_EFF_DT,
  TARGET.DIST_MGR_NAME = SOURCE.NEW_DIST_MGR_NAME1,
  TARGET.SITE_DM_END_DT = SOURCE.SITE_DM_END_DT,
  TARGET.CURRENT_DIST_MGR_CD = SOURCE.CURRENT_DIST_MGR_CD,
  TARGET.UPDATE_TSTMP = SOURCE.UPDATE_TSTMP,
  TARGET.LOAD_TSTMP = SOURCE.LOAD_TSTMP
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_DELETE"
  AND TARGET.DIST_MGR_NAME = SOURCE.NEW_DIST_MGR_NAME1
  AND TARGET.SITE_DM_END_DT = SOURCE.SITE_DM_END_DT
  AND TARGET.CURRENT_DIST_MGR_CD = SOURCE.CURRENT_DIST_MGR_CD
...
INSERT
  (
    TARGET.LOCATION_ID,
    TARGET.SITE_DM_EFF_DT,
    TARGET.DIST_MGR_NAME,
    TARGET.SITE_DM_END_DT,
    TARGET.CURRENT_DIST_MGR_CD,
    TARGET.UPDATE_TSTMP,
    TARGET.LOAD_TSTMP
  )
VALUES
  (
    SOURCE.NEW_LOCATION_ID1,
    SOURCE.SITE_DM_EFF_DT,
    SOURCE.NEW_DIST_MGR_NAME1,
    SOURCE.SITE_DM_END_DT,
    SOURCE.CURRENT_DIST_MGR_CD,
    SOURCE.UPDATE_TSTMP,
    SOURCE.LOAD_TSTMP
  )""")
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_Site_Dm_Hist")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_Site_Dm_Hist", mainWorkflowId, parentName)
