...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_store_rank_STORE_DECILE")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_store_rank_STORE_DECILE", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_to_DM_DEPT_SEGMENTS_0
...
df_0.createOrReplaceTempView("Shortcut_to_DM_DEPT_SEGMENTS_0")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_USER_RANKING_HIERARCHY_1
...
df_1.createOrReplaceTempView("Shortcut_to_USER_RANKING_HIERARCHY_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_DAY_SKU_STORE_2
...
df_2.createOrReplaceTempView("Shortcut_to_SALES_DAY_SKU_STORE_2")
...
query_5 = f"""SELECT
  ...
  TT.STORE_NBR AS STORE_NBR,
  U.HIERARCHY_SUB_LVL AS HIERARCHY_SUB_LVL,
  U.HIERARCHY_NAME AS HIERARCHY_NAME,
  U.RANK_NAME AS RANK_NAME,
  U.RANK_CRITERIA AS RANK_CRITERIA,
  U.RANK_VALUE_TYPE AS RANK_VALUE_TYPE,
  U.RANK_MIN_VALUE AS RANK_MIN_VALUE,
  U.RANK_MAX_VALUE AS RANK_MAX_VALUE,
  CURRENT_DATE AS LOAD_DT,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_USER_RANKING_HIERARCHY_1 U,
  (
    SELECT
      LOCATION_ID,
      STORE_NBR,
    ...
    FROM
      (
        SELECT
          ...
          SUM(ROUND((T1.NSAMT / T2.TOT_NSAMT) * 100, 1)) OVER (
            PARTITION BY
              T1.CONSUM_ID
            ORDER BY
              T1.NSAMT DESC
            ROWS
              BETWEEN 5000 PRECEDING
              AND CURRENT ROW
          ) AS CUME_SUM
        FROM
          (
            SELECT
              ...
              SUM(
                SALES_AMT - RETURN_AMT - DISCOUNT_AMT + DISCOUNT_RETURN_AMT - POS_COUPON_AMT - SPECIAL_SALES_AMT + SPECIAL_RETURN_AMT + SPECIAL_SRVC_AMT
              ) AS NSAMT
            FROM
              SALES_DAY_SKU_STORE W,
              Shortcut_to_SITE_PROFILE_4 S,
              SKU_PROFILE P,
              DM_DEPT_SEGMENTS D
            WHERE
              W.WEEK_DT >= (
                (CURRENT_DATE + 1 - DATE_PART('DOW', CURRENT_DATE)) - 51 * 7
              )
...
SELECT
  ...
  LOAD_DT AS LOAD_DT
FROM
  SQ_Shortcut_to_USER_RANKING_HIERARCHY_5""")

# COMMAND ----------
#Post session variable updation
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_store_rank_STORE_DECILE")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_store_rank_STORE_DECILE", mainWorkflowId, parentName)
