...
updateVariable(preVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_store_data")

# COMMAND ----------
fetchAndCreateVariables(parentName,"m_store_data", variablesTableName, mainWorkflowId)

# COMMAND ----------
# DBTITLE 1, Shortcut_to_STORE_DATA_FLAT_0
...
df_0.createOrReplaceTempView("Shortcut_to_STORE_DATA_FLAT_0")
...
query_2 = f"""SELECT
  ...
  MERCH_ID AS MERCH_ID,
  TAX_JURISDICTION_CD AS TAX_JURISDICTION_CD,
  TO_DATE(EARLIEST_VALUE_POST_DT, 'YYYYMMDD') AS o_EARLIEST_VALUE_POST_DT,
  TO_DECIMAL(TOT_VALUE_WARNING_LMT, 2) AS o_TOT_VALUE_WARNING_LMT,
  TO_DECIMAL(TOT_VALUE_ERROR_LMT, 2) AS o_TOT_VALUE_ERROR_LMT,
  TRANS_CNT_LOWER_LMT AS TRANS_CNT_LOWER_LMT,
  TRANS_CNT_WARNING_LMT AS TRANS_CNT_WARNING_LMT,
  TRANS_CNT_ERROR_LMT AS TRANS_CNT_ERROR_LMT,
  TRANS_VALUE_ERROR_LMT AS TRANS_VALUE_ERROR_LMT,
  TRANS_SEQ_GAP_CNT_ERROR_LMT AS TRANS_SEQ_GAP_CNT_ERR_LMT,
  TO_DECIMAL(OUT_OF_BALANCE_CNT, 2) AS o_OUT_OF_BALANCE_CNT,
  TO_DECIMAL(OUT_OF_BALANCE_VALUE, 2) AS o_OUT_OF_BALANCE_VALUE,
  TO_DECIMAL(BANK_DEPOSIT_VAR_PCT, 2) AS o_BANK_DEPOSIT_VAR_PCT,
  TO_DATE(CREATE_DT, 'YYYYMMDD') AS o_CREATE_DT,
  CREATE_USER AS CREATE_USER,
  TO_DATE(UPDATE_DT, 'YYYYMMDD') AS o_UPDATE_DT,
  UPDATE_USER AS UPDATE_USER,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
...
updateVariable(postVariableAssignment, variablesTableName, mainWorkflowId, parentName, "m_store_data")

# COMMAND ----------
#Update Mapping Variables in database.
persistVariables(variablesTableName, "m_store_data", mainWorkflowId, parentName)
