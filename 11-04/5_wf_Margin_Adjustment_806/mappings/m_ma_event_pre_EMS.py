...
df_5=spark.sql("""SELECT (ROW_NUMBER() OVER(ORDER BY(SELECT NULL))-1) * (SELECT Increment_By FROM SEQ_MA_EVENT_ID) + (SELECT NEXTVAL FROM SEQ_MA_EVENT_ID) AS MA_EVENT_ID,
...
SEQ_MA_EVENT_ID.Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM SEQ_MA_EVENT_ID INNER JOIN ASQ_MA_EVENT_PRE_NEW_4 ON SEQ_MA_EVENT_ID.Monotonically_Increasing_Id = ASQ_MA_EVENT_PRE_NEW_4.Monotonically_Increasing_Id
""")
...
MA_MAX_AMT AS MA_MAX_AMT,
null AS UPDATE_DT,
null AS LOAD_DT FROM UNI_EMS_7""")