...
df_5=spark.sql("""SELECT OVRD_CASH_DISCOUNT_PCT1 AS OVRD_CASH_DISCOUNT_PCT1,
IIF(ISNULL(FISCAL_MO1),FISCAL_MO,FISCAL_MO1) AS FISCAL_MO,
IIF(ISNULL(SOURCE_VENDOR_ID1),SOURCE_VENDOR_ID,SOURCE_VENDOR_ID1) AS SOURCE_VENDOR_ID,
SYSDATE AS UPDATE_TSTMP,
IIF(ISNULL(LOAD_TSTMP),SYSDATE,LOAD_TSTMP) AS LOAD_TSTMP,
IIF(ISNULL(FISCAL_MO),'I',
IIF(IIF(ISNULL(OVRD_CASH_DISCOUNT_PCT1),-99,OVRD_CASH_DISCOUNT_PCT1)
<>
IIF(ISNULL(OVRD_CASH_DISCOUNT_PCT),-99,OVRD_CASH_DISCOUNT_PCT),'U','X')) AS INS_UPD_FLAG,
Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM JNR_MA_CASH_DISCOUNT_OVRD_4""")
