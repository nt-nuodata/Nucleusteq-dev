...
df_5=spark.sql("""SELECT IIF(ISNULL(WEEK_DT),WEEK_DT_OLD,WEEK_DT) AS WEEK_DT_OUT,
IIF(ISNULL(LOCATION_ID),LOCATION_ID_OLD,LOCATION_ID) AS LOCATION_ID_OUT,
IIF(ISNULL(SAP_DEPT_ID),SAP_DEPT_ID_OLD,SAP_DEPT_ID) AS SAP_DEPT_ID_OUT,
IIF(ISNULL(MARGIN_RATE),0,MARGIN_RATE) AS MARGIN_RATE_OUT,
SYSDATE AS UPDATE_TSTMP,
IIF(ISNULL(LOAD_TSTMP_OLD),SYSDATE,LOAD_TSTMP_OLD) AS LOAD_TSTMP,
IIF(ISNULL(WEEK_DT_OLD),DD_INSERT,
IIF(
IIF(ISNULL(MARGIN_RATE_OLD),-1,  MARGIN_RATE_OLD)
<> 
IIF(ISNULL(MARGIN_RATE),-1, MARGIN_RATE)
,DD_UPDATE,
DD_REJECT)) AS UPDATE_STRATEGY,
Monotonically_Increasing_Id AS Monotonically_Increasing_Id FROM JNR_SERVICES_MARGIN_4""")
